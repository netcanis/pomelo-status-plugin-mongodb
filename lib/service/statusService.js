...
StatusService.prototype.getRecordsByUids = function(uids, cb) {
  ...
  var userInfoModel = this.mongoose.model('userInfo');
  var records = [];
  userInfoModel.find({"uid":{"$in":uids}}).exec(function (err, results) {
    var ret = results || [];
    if(ret == []){
...
StatusService.prototype.getRecordsByOrg = function(orgId, cb) {
  ...
  var userInfoModel = this.mongoose.model('userInfo');
  var records = [];
  userInfoModel.find({"orgs.orgId":orgId}).exec(function (err, results) {
    var ret = results || [];
    if(ret == []){
...
StatusService.prototype.getRecordsByGroup = function(groupId, cb) {
  ...
  var userInfoModel = this.mongoose.model('userInfo');
  var records = [];
  userInfoModel.find({"groups.groupId":groupId}).exec(function (err, results) {
    var ret = results || [];
    if(ret == []){
